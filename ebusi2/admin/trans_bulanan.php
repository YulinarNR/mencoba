<?php
  ...
  ?>
        ...
        <div class="container-fluid">
          ...
          $banyak_hari = mysqli_query($koneksi, "SELECT DAY(LAST_DAY('$year-01')) AS BANYAK_HARI");
          ...
          for($i=1; $i<=$row_hari; $i++){
            ...
            $result_hari[$i] = mysqli_query($koneksi, "SELECT SUM(total_pembelian) as TOT_OMSET FROM tb_pembelian WHERE (status_pembelian = 'Barang Dikirim' OR status_pembelian = 'Sudah Kirim Pembayaran') AND tanggal_pembelian LIKE '%$day%'");
          ...
          $rjml_pesanan_bulan = mysqli_query($koneksi, "SELECT COUNT(*) as total_pembelian FROM tb_pembelian WHERE tanggal_pembelian LIKE '%$year%'");
          $djml_pesanan_bulan = mysqli_fetch_assoc($rjml_pesanan_bulan);

          // JUMLAH PRODUK YANG DIPESAN BULAN INI
          $rjml_produk_bulan = mysqli_query($koneksi, "SELECT COUNT(*) as TOT_PRODUK FROM tb_pembelian WHERE tanggal_pembelian LIKE '$year%'");
          $djml_produk_bulan = mysqli_fetch_assoc($rjml_produk_bulan);

          // JUMLAH OMSET BULAN INI
          $romset_bulan = mysqli_query($koneksi, "SELECT SUM(total_pembelian) as TOT_OMSET FROM tb_pembelian WHERE (status_pembelian = 'Barang Dikirim' OR status_pembelian = 'Sudah Kirim Pembayaran') AND  tanggal_pembelian LIKE '%$year%'");
                ...
                <div class="card-body">
                  <div class="row no-gutters align-items-center">
                    <div class="col mr-2">
                      ...
                      <div class="h5 mb-0 font-weight-bold text-gray-800"><span class="Count"><?=number_format($djml_pesanan_bulan['total_pembelian'], 0,".",".")?></span> Pesanan</div>
                    </div>
                    <div class="col-auto">
